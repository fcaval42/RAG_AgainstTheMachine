...
def chunk_text_markdown(file_path: str, paragraphes: list[str],
    ...
    for para in paragraphes:
        ...
        # paragraphe seul déjà trop grand : on le re-découpe ligne par ligne
        if len(para) > max_size:
            if current:
                chunks.append((file_path, current_start,
                               current_start + len(current), current))
                current = ""
            for fp, s, e, t in chunk_text_size(file_path, para, max_size):
                chunks.append((fp, position + s, position + e, t))
            current_start = position + len(para) + 2
        else:
            if current:
                current += "\n\n" + para
            else:
